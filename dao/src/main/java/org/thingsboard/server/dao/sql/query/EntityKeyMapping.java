...
public class EntityKeyMapping {
        ...
        if (entityKey.getType().equals(EntityKeyType.ENTITY_FIELD)) {
            if (entityKey.getKey().equals("entityType") && !filterType.equals(EntityFilterType.RELATIONS_QUERY)) {
                ...
                if (existingEntityFields.contains(alias)) {
                    ...
                    return String.format("cast(e.%s as char) as %s", column, getValueAlias());
                } else {
    ...
    private String buildTimeSeriesOrAttrSelection(boolean attr) {
        ...
        String attrValSelection =
                String.format("(ifnull(cast(%s.bool_v as char), '') || " +
                        "ifnull(%s.str_v, '') || " +
                        "ifnull(cast(%s.long_v as char), '') || " +
                        "ifnull(cast(%s.dbl_v as char), '') || " +
                        "ifnull(cast(%s.json_v as char), '')) as %s", alias, alias, alias, alias, alias, attrValAlias);
        ...
        if (this.isSortOrder) {
            ...
            String attrSortOrderSelection =
                    String.format("ifnull(%s.dbl_v, cast(%s.long_v as double precision), (case when %s.bool_v then 1 else 0 end)) %s," +
                            "ifnull(%s.str_v, cast(%s.json_v as char), '') %s", alias, alias, alias, attrNumAlias, alias, alias, attrVarcharAlias);

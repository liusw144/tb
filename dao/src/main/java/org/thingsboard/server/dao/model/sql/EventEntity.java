...
public abstract class EventEntity<T extends Event> implements BaseEntity<T> {
    ...
    @Id
    @Column(name = ModelConstants.ID_PROPERTY, columnDefinition = "varchar(36)")
    @Type(type = "uuid-char")
    protected UUID id;

    @Column(name = EVENT_TENANT_ID_PROPERTY, columnDefinition = "varchar(36)")
    @Type(type = "uuid-char")
    protected UUID tenantId;

    @Column(name = EVENT_ENTITY_ID_PROPERTY, columnDefinition = "varchar(36)")
    @Type(type = "uuid-char")

...
//                    "ON CONFLICT (entity_id, key) DO UPDATE SET ts = ?, bool_v = ?, str_v = ?, long_v = ?, dbl_v = ?, json_v = cast(? AS json)";
        ...
        private static final String INSERT_OR_UPDATE =
            "REPLACE INTO ts_kv_latest (entity_id, `key`, ts, bool_v, str_v, long_v, dbl_v,  json_v) VALUES(?, ?, ?, ?, ?, ?, ?, cast(? AS json))";
//        private static final String INSERT_OR_UPDATE =
//            "INSERT INTO ts_kv_latest (entity_id, `key`, ts, bool_v, str_v, long_v, dbl_v,  json_v) VALUES(?, ?, ?, ?, ?, ?, ?, cast(? AS json)) " +
//                    "ON DUPLICATE KEY UPDATE ts = ?, bool_v = ?, str_v = ?, long_v = ?, dbl_v = ?, json_v = cast(? AS json)";
...
//    private static final String INSERT_OR_UPDATE_BY_LATEST_TS = INSERT_OR_UPDATE + " WHERE ts_kv_latest.ts <= ?";
    //改用存储过程ts_kv_latest_insert_or_update_by_latest_ts实现
    private static final String INSERT_OR_UPDATE_BY_LATEST_TS = " call ts_kv_latest_insert_or_update_by_latest_ts(?,?,?,?,?,?,?,?)";
    @Override
            ...
            protected void doInTransactionWithoutResult(TransactionStatus status) {
                ...
                jdbcTemplate.batchUpdate(insertOrUpdateQuery, new BatchPreparedStatementSetter() {
                    ...
                    public void setValues(PreparedStatement ps, int i) throws SQLException {
                        ...
                        ps.setString(1, tsKvLatestEntity.getEntityId()+"");
